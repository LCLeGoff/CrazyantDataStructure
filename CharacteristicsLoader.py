	...
	def build(self, name):
		...
		if name.name in ['session', 'trial', 'n_frames', 'fps', 'mm2px', 'food_radius']:
			res = pd.DataFrame(
				...
				index=np.array(list(chara_dict.keys()), dtype=float),
				columns=[name.name])
			...
			res = pd.DataFrame(
				[chara_dict[key][name.name][0]+chara_dict[key][name.name][1] for key in chara_dict.keys()],
				index=np.array(list(chara_dict.keys()), dtype=float),
				columns=['x1', 'y1', 'x2', 'y2'])
			res.index.name = 'id_exp'
		elif name.name in ['entrance']:
			res = pd.DataFrame(
				[chara_dict[key][name.name][0]+chara_dict[key][name.name][1]+chara_dict[key][name.name][2]
